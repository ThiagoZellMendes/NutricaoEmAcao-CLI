...
export function calcularGorduraCorporal(genre: Sexo, idade: number, dobras: DobraCutanea): ResultadoGordura {
  ...
  const categoria = classificarPercentualGordura(genre, idade, percentualGordura);
...
function classificarPercentualGordura(genre: Sexo, idade: number, percentualGordura: number): string {
  ...
  for (const faixaEtaria of Object.keys(tabelaClassificacao)) {
    ...
    if (idade >= min && idade <= max) {
      ...
      for (const [categoria, [minPercentual, maxPercentual]] of Object.entries(classificacoes)) {
        if (percentualGordura >= minPercentual && percentualGordura <= maxPercentual) {
          return categoria;
        }
        // Adicionando uma margem de 1% para considerar uma faixa próxima
        if (percentualGordura > minPercentual - 1 && percentualGordura < maxPercentual + 1) {
          return categoria;
        }
      }
    }
...
function obterTabelaClassificacao(genre: Sexo, idade: number): Record<string, Record<string, [number, number]>> {
  const tabelaClassificacaoMasculino: Record<string, Record<string, [number, number]>> = {
    '16 a 25': {
      'Muito Baixo': [0, 4],
      Excelente: [4, 6],
      'Muito Bom': [7, 10],
  ...
  const tabelaClassificacaoFeminino: Record<string, Record<string, [number, number]>> = {
    '16 a 25': {
      'Muito Baixo': [0, 13],
      Excelente: [13, 17],
      'Muito Bom': [17, 19],
  ...
  return genre === Sexo.Masculino ? tabelaClassificacaoMasculino : tabelaClassificacaoFeminino;
}

function calcularDensidadeCorporal(genre: Sexo, dobras: DobraCutanea, idade: number): number {
  const somaDobras = dobras.triceps + dobras.subescapular + dobras.triceps + dobras.supraIliaca;
