...
import { Alert, Dimensions, TextInput, findNodeHandle } from 'react-native';
...
import { ButtonComponent, DropdownComponent, FeedbackModal, InputForm, LoadingModal } from '../../../components';
...
export function RegisterPatients() {
  ...
  async function handleCreatePatient({ fullName, cpf, age }: PatientProps) {
    ...
    try {
      ...
      const cpfSnapshot = await firestore().collection('patients').where('cpf', '==', cpf).get();
